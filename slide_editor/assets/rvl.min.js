...
*/
window.RVL = {}, window.RVL.views = {}, window.RVL.tid1=null,window.RVL.tid2=null,
$(function() {
	function e() {
...
RVL.init = function() {
    var t = $(document.body);
    RVL.views.edit();//t.hasClass("index-view") ? RVL.views.index() : t.hasClass("registration-view") ? RVL.views.registration() : t.hasClass("user-view") ? RVL.views.user() : t.hasClass("deck-view") ? RVL.views.deck() : t.hasClass("edit-view") ? RVL.views.edit() : RVL.views.text()
...
RVL.util = function() {
            ...
            open: function(n, r) {
                if (n === "create") {
					...
					if (btnname =="Replace" && new_content) {
						...
						$(".reveal .slides").replaceWith(new_content);
						RVL.modal.close();
						RVL.init();
	...
	if(RVL.tid1) {
		//alert("clear tid1");
		clearInterval(RVL.tid1);
	}
	if(RVL.tid2) {
		//alert("clear tid2");
		clearInterval(RVL.tid2);
	}
	
	RVL.tid1=setInterval(ll_local, 1e3),
	load_localstorage(),
        $('#toolbar').html(""),
    o.setPanel("toolbar"), u(), a(), v(), d(), b(), g(), i = p(), RVL.tid2=setInterval(l, 1e3), head.js("./assets/highlight.js", function() {
        hljs.initHighlightingOnLoad()
    }), 
			$(".reveal .slides section.present").off(),
			$(".reveal .slides section.present").live("focus", E), 
			$(".reveal .slides section.present").live("blur", S), 
			$(".options .save-deck").off(),
			$(".options .save-deck").on("click", k), 
			$(".options .save-deck-local").off(),
			$(".options .save-deck-local").on("click", kk_local), 
			$(".options .import-deck").off(),
			$(".options .import-deck").on("click", import_deck), 
			$(".options .clean-local").off(),
			$(".options .clean-local").on("click", clean_local), 
			$(".options .download-deck").off(),
			$(".options .download-deck").on("click", C), 
			$(".options .preview-deck").off(),
			$(".options .preview-deck").on("click", L), 
			$(".options .settings").off(),
			$(".options .settings").on("click", A), 
			$(".options .publish").off(),
			$(".options .publish").live("click", O), 
			$(".options .unpublish").off(),
			$(".options .unpublish").live("click", M), 
			$("#modal .settings .theme li").off(),
			$("#modal .settings .theme li").on("click", _), 
			$("#modal .settings .transition li").off(),
			$("#modal .settings .transition li").on("click", D), 
			$(window).off(),  // TODO: not safe
			$(window).on("keyup", P), 
			$(window).on("beforeunload", H), 
			$(window).on("resize", B), 
			e.off(),
			e.on("click", x), 
			t.off(),
